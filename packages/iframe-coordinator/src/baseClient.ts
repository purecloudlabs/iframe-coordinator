 ...
 */
export abstract class IfcClient<T extends Window | DedicatedWorkerGlobalScope> {
   ...
   * {@link IfcClient.messaging | messaging} interface.
   *
  ...
  /**
   * Removes an event listener previously registered with {@link IfcClient.addListener | addListener}.
  ...
  /**
   * Removes all event listeners previously registered with {@link IfcClient.addListener | addListener}.
   * @param type A string which specifies the type of event for which to remove an event listener.
   */
  public removeAllListeners(type: "environmentalData"): IfcClient<T> {
...
bad input into one of the iframe-coordinator client methods.
  ...
  /*
   * Client-specific method for delivering a validated payload to the host application
   */
