 ...
 */
export abstract class BaseClient<T extends Window | DedicatedWorkerGlobalScope> {
   ...
   * {@link BaseClient.messaging | messaging} interface.
   *
  ...
  /**
   * Removes an event listener previously registered with {@link BaseClient.addListener | addListener}.
  ...
  /**
   * Removes all event listeners previously registered with {@link BaseClient.addListener | addListener}.
   * @param type A string which specifies the type of event for which to remove an event listener.
   */
  public removeAllListeners(type: "environmentalData"): BaseClient<T> {
