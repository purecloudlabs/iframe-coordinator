...
listener for environment data passed from the host application, and start the client.
 ...
 client.EnvData \| Environment data is data set in the host application that is static over time, but that clients will
...
You can listen for messages from the host on a topic via the  client.EventEmitter \| messaging API
